...
export const remove = async (req, res) => {
  let removed = await Hotel.findByIdAndDelete(req.params.hotelId)
    .select("-image.data")
    .exec();
  res.json(removed); // update to display hotel name
};
...
export const userHotelBookings = async (req, res) => {
  try {
    ...
    console.log("user Hotel BOoking display error: ", err);
  }
};
...
export const isAlreadyBooked = async (req, res) => {
  ...
  const userOrders = await Order.find({ orderedBy: req.user._id })
    .select("hotel")
    .exec();
  // Check if hotel id is found in userOrders
  let ids = [];
...
export const searchListings = async (req, res) => {
  ...
  // console.log("hit search endpooint", location, date, bed);
